...
class Loot
{
    ...
    public:
        ...
        void virtual AddItem(LootStoreItem const& item);
        void virtual AddItem(uint32 itemid, uint32 count, uint32 randomSuffix, int32 randomPropertyId);             // used in item.cpp to explicitly load a saved item
